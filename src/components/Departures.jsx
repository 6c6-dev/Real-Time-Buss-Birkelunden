...
export default () => {
  ...
  const getDepartures = async () => {
    getNydalenDepartures().then(nydalenDeps => {
      const filteredDeps = filterDepartures(nydalenDeps, "Birkelunden")
  ...
  return (
    <div style={{
      ...
      <div style={{display: "grid", gridTemplateColumns: "1fr 1fr", columnGap: "3vw"}}>
        <h1 style={{color: "white", textAlign: "right"}}>Birkelunden  </h1>
        <h1 style={{color: "gray", textAlign: "right", fontSize:"4ch", margin: "0.1%", position:"absolute", marginLeft:"80%"}}>Oppdatert: {whenLoaded}</h1>
      </div>
      {deps.map((dep, i) => {
        if(i < 10) {
